...
%
  ...
  c('summerbalyes') = toInt(linePosOffset(24, 1));
  c('winterjdbeg') = toInt(linePosOffset(25, 1));
  c('winterjdend') = toInt(linePosOffset(26, 1));
  ...
  c('namedgmthickness') = (lineposOffset(64, 1));
  c('nameklima') = (lineposOffset(65, 1));

  c('laenge') = str2double(lineposOffset(67, 1));
  c('breite') = str2double(lineposOffset(68, 1));
  c('reflongitude') = str2double(lineposOffset(69, 1));
  c('rowclim') = toInt(lineposOffset(70, 1));
  c('colclim') = toInt(lineposOffset(71, 1));
  c('climoutsideyes') = toInt(lineposOffset(72, 1));
  c('heightclim') = toInt(lineposOffset(72, 2));
  c('gridsize') = toInt(lineposOffset(73, 1));
  c('timestep') = toInt(lineposOffset(74, 1));

  c('formatclimdata') = toInt(lineposOffset(76, 1));
  c('maxcol') = toInt(lineposOffset(77, 1));

  c('coltemp') = toInt(lineposOffset(78, 1));
  c('colhum') = toInt(lineposOffset(79, 1));
  c('colwind') = toInt(lineposOffset(80, 1));
  c('colglob') = toInt(lineposOffset(81, 1));
  c('colref') = toInt(lineposOffset(82, 1));
  c('colnet') = toInt(lineposOffset(83, 1));
  c('collongin') = toInt(lineposOffset(84, 1));
  c('collongout') = toInt(lineposOffset(85, 1));
  c('colprec') = toInt(lineposOffset(86, 1));
  c('colcloud') = toInt(lineposOffset(87, 1));
  c('coltempgradvarying') = toInt(lineposOffset(88, 1));
  c('coldis') = toInt(lineposOffset(89, 1));

  c('ERAtempshift') = toInt(lineposOffset(91, 1));
  c('ERAwindshift') = toInt(lineposOffset(92, 1));

  c('methodtempinterpol') = toInt(lineposOffset(94, 1));
  c('tempgrad') = str2double(lineposOffset(95, 1));
  c('tempscenario') = toInt(lineposOffset(96, 1));
  c('precscenario') = toInt(lineposOffset(97, 1));
  c('monthtempgradyes') = toInt(lineposOffset(99, 1));

  c('monthtempgrad') = arrayfun(@(y)str2double(lineposOffset(99, y)), 2:13);

  c('monthtempscenyes') = toInt(lineposOffset(100, 1));
  c('monthtempscen') = arrayfun(@(y)str2double(lineposOffset(100, y)), 2:13);
  c('monthprecipscenyes') = toInt(lineposOffset(101, 1));
  c('monthprecipscen') = arrayfun(@(y)str2double(lineposOffset(101, y)), 2:13);


  c('n_albfiles') = toInt(lineposOffset(103, 1));
  ...
  c('albmin') = str2double(lineposOffset(109, 1));
  c('snowalbincrease') = str2double(lineposOffset(110, 1));
  c('albiceproz') = str2double(lineposOffset(111, 1));
  c('ndstart') = toInt(lineposOffset(112, 1));

  c('split') = toInt(lineposOffset(114, 1));
  c('prozdiffuse') = str2double(lineposOffset(115, 1));
  c('trans') = str2double(lineposOffset(116, 1));
  c('ratio') = str2double(lineposOffset(117, 1));
  c('ratiodir2dir') = str2double(lineposOffset(118, 1));
  c('surftemplapserate') = str2double(lineposOffset(119, 1));
  c('directfromfile') = toInt(lineposOffset(120, 1));
  c('pathdirectfile') = (lineposOffset(121, 1));
  c('daysdirect') = toInt(lineposOffset(122, 1));
  c('slopestation') = str2double(lineposOffset(123, 1));

  c('iterstep') = str2double(lineposOffset(125, 1));
  c('z0wice') = str2double(lineposOffset(126, 1));
  c('dividerz0T') = str2double(lineposOffset(127, 1));
  c('dividerz0snow') = str2double(lineposOffset(128, 1));
  c('z0proz') = str2double(lineposOffset(129, 1));
  c('icez0min') = str2double(lineposOffset(130, 1));
  c('icez0max') = str2double(lineposOffset(131, 1));

  c('methodprecipinterpol') = toInt(lineposOffset(133, 1));
  c('precgrad') = str2double(lineposOffset(134, 1));
  c('precgradhigh') = str2double(lineposOffset(135, 1));
  c('precgradelev') = toInt(lineposOffset(135, 2));
  c('preccorr') = str2double(lineposOffset(136, 1));
  c('snowmultiplierglacier') = str2double(lineposOffset(137, 1));
  c('snowmultiplierrock') = str2double(lineposOffset(138, 1));
  c('threshtemp') = str2double(lineposOffset(139, 1));
  c('onlyglacieryes') = toInt(lineposOffset(140, 1));
  c('glacierpart') = str2double(lineposOffset(141, 1));

  c('nameqcalc') = (lineposOffset(143, 1));
  c('nodis') = toInt(lineposOffset(144, 1));
  c('firnkons') = toInt(lineposOffset(145, 1));
  c('snowkons') = toInt(lineposOffset(146, 1));
  c('icekons') = toInt(lineposOffset(147, 1));
  c('rockkons') = toInt(lineposOffset(148, 1));

  c('qfirnstart') = str2double(lineposOffset(150, 1));
  c('qsnowstart') = str2double(lineposOffset(151, 1));
  c('qicestart') = str2double(lineposOffset(152, 1));
  c('qrockstart') = str2double(lineposOffset(153, 1));
  c('qground') = str2double(lineposOffset(154, 1));
  c('jdstartr2diff') = str2double(lineposOffset(155, 1));

  c('percolationyes') = toInt(lineposOffset(157, 1));
  c('slushformationyes') = toInt(lineposOffset(158, 1));
  c('densificationyes') = toInt(lineposOffset(159, 1));
  c('wetstartyes') = toInt(lineposOffset(160, 1));
  c('ndepths') = toInt(lineposOffset(161, 1));
  c('factinter') = toInt(lineposOffset(162, 1));

  c('thicknessfirst') = str2double(lineposOffset(164, 1));
  c('thicknessdeep') = str2double(lineposOffset(165, 1));
  c('depthdeep') = str2double(lineposOffset(166, 1));
  c('denssnow') = toInt(lineposOffset(167, 1));
  c('irrwatercontyes') = toInt(lineposOffset(168, 1));
  c('irrwatercont') = str2double(lineposOffset(169, 1));

  c('factsubsurfout') = toInt(lineposOffset(171, 1));
  c('offsetsubsurfout') = toInt(lineposOffset(172, 1));

  c('runoffyes') = toInt(lineposOffset(174, 1));
  c('superyes') = toInt(lineposOffset(174, 2));
  c('wateryes') = toInt(lineposOffset(174, 3));
  c('surfwateryes') = toInt(lineposOffset(174, 4));
  c('slushyes') = toInt(lineposOffset(174, 5));
  c('coldsnowyes') = toInt(lineposOffset(174, 6));
  c('coldtotyes') = toInt(lineposOffset(174, 7));

  c('ddmethod') = toInt(lineposOffset(178, 1));
  c('DDFice') = str2double(lineposOffset(179, 1));
  c('DDFsnow') = str2double(lineposOffset(180, 1));

  c('meltfactor') = str2double(lineposOffset(182, 1));
  c('radfactorice') = str2double(lineposOffset(183, 1));
  c('radfactorsnow') = str2double(lineposOffset(184, 1));
  c('debrisfactor') = toInt(lineposOffset(185, 1));

  c('coordinatesyes') = toInt(lineposOffset(187, 1));

  %Read in the stake coords
  if c('maxmeltstakes') > 0
    fmt = c('coordinatesyes');
    stake_coords = [];
    for ii = 188:(188 + c('maxmeltstakes') - 1)
